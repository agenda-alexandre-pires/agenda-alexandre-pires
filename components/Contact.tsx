...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // CONFIGURAÇÃO DE AUTOMACAO (N8N)
    const WEBHOOK_URL = 'https://n8nwebhook.emersonporfaaudio.com.br/webhook/contato-site'; 
    ...
    try {
      ...
      // FALLBACK: Se falhar o n8n, abrimos o mailto para segurança do contato
      const subject = `Contato Site (Urgente): ${formData.service}`;
      const body = `Houve um erro no envio automático n8n, segue cópia manual:%0D%0A%0D%0ANome: ${formData.name}%0D%0ATelefone: ${formData.phone}%0D%0AE-mail: ${formData.email}%0D%0AServiço: ${formData.service}%0D%0A%0D%0AMensagem:%0D%0A${formData.message}`;
            ...
            {status === 'success' ? (
              <div className="h-full flex flex-col items-center justify-center text-center py-12 animate-fade-in min-h-[400px]">
                ...
                <p className="text-neutral-400 max-w-sm">
                    Seus dados foram processados com sucesso via n8n. Entrarei em contato em breve.
                </p>
